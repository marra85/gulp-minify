...
module.exports = function (opt) {
  ...
  async function minify(file, encoding, callback) {
    ...
    if (file.isStream()) {
      this.emit("end");
      return new callback(
        PluginError("gulp-minify", "Streaming not supported:" + file.path)
      );
    }
    ...
    options.fromString = options.hasOwnProperty("fromString")
      ? options.fromString
      : true;
    ...
    let min_file = new Vinyl({
      base: file.base,
      path: Array.isArray(ext.min)
        ? file.path.replace(ext.min[0], ext.min[1])
        : file.path.replace(jsExtensions, ext.min),
    });
    ...
    try {
      mangled = await terser.minify(String(file.contents), uglifyOptions);
      min_file.contents = Buffer.from(
        mangled.code.replace(reSourceMapComment, "")
      );
    } catch (e) {
      this.emit("end");
